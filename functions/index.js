 ...
 * Deletes both Firestore document and Firebase Authentication user
 */
exports.deleteVoterPermanently = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    // Delete Firebase Authentication user if uid is provided
    if (uid) {
      try {
    ...
    return {
      success: true,
      message: 'Voter permanently deleted from system.',
    };
  } catch (error) {
